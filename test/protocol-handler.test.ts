...
describe('协议处理器 (ProtocolHandler)', () => {
  ...
  test('应当能强制踢出玩家', async () => {
    const mockRoom: Partial<Room> = {
      id: 'room1',
      players: new Map([
        [123, { user: { id: 123, name: 'Target', monitor: false }, connectionId: 'conn1' } as any],
      ]),
      ownerId: 456,
    };
    ...
    mockAuthService.authenticate = jest
      .fn()
      .mockResolvedValue({ id: 123, name: 'Target', monitor: false });
    ...
    const mockRoom: Partial<Room> = {
      ...
      players: new Map([
        [
          123,
          {
            user: { id: 123, name: 'Host', monitor: false },
            isReady: false,
            connectionId: 'conn1',
          } as any,
        ],
