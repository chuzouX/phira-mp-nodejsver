  ...
  try {
    ...
    setTimeout(() => {
      ctx.broadcastToRoom(room, {
        ...
        user: { id: -1, name: ctx.serverName, avatar: ctx.defaultAvatar, monitor: true },
      });
    ...
    ctx.logger.error(
      `创建房间失败: ${connectionId} (用户: ${session.userId}, 房间: ${roomId}, 错误: ${errorMessage})`,
...
export function handleJoinRoom(
  ...
  if (!room) {
    if (ctx.federationManager?.isRemoteRoom(roomId)) {
      ctx.logger.info(`[联邦] 玩家 ${session.userId} 尝试加入远程房间 ${roomId}`, {
        userId: session.userId,
      });
      ctx.federationManager.proxyJoinRoom(
        session.userId,
  ...
  if (success) {
    ctx.logger.info(
      `玩家 "${session.userInfo.name}" (ID: ${session.userId}) 加入了房间 "${roomId}"`,
      { userId: session.userId },
    );
    if (monitor && !room.live) {
      room.live = true;
      ctx.logger.info(`房间 "${roomId}" 已进入 live 模式`, { userId: session.userId });
    }
    ...
    if (room.state.type === 'Playing') {
      ...
      if (joinedPlayer) {
        ...
        setTimeout(() => {
          ctx.sendCommandToUser(userId, {
            type: ServerCommandType.Message as any,
            message: { type: 'Chat', user: -1, content: '此房间正在游戏中，请等待游戏结束' },
          });
        }, 2000);
      }
    }
...
export function handleLeaveRoom(
  ...
  ctx.logger.info(
    `玩家 "${session.userInfo.name}" (ID: ${session.userId}) 离开了房间 "${room.id}"`,
    { userId: session.userId },
  );
  ...
  ctx.pluginManager?.emit('room:leave', {
    roomId: room.id,
    userId: session.userId,
  ...
  if (ctx.federationManager?.getConfig?.()?.enabled) {
    if (!updatedRoom) {
      ctx.federationManager.broadcastRoomEvent('room_deleted', room.id, null).catch(() => {});
    } else {
      ctx.federationManager
        .broadcastRoomEvent(
          'room_updated',
          room.id,
          ctx.federationManager.buildLocalRoomInfo(updatedRoom),
        )
        .catch(() => {});
    }
  }
  if (updatedRoom && wasHost && updatedRoom.ownerId !== session.userId) {
    ...
    for (const playerInfo of updatedRoom.players.values()) {
      ...
      if (callback) {
        callback({ type: ServerCommandType.ChangeHost, isHost });
      }
    }
  }
  if (updatedRoom && wasPlaying) {
    ctx.checkGameEnd(updatedRoom);
  }
  ctx.respond(connectionId, sendResponse, {
    type: ServerCommandType.LeaveRoom,
    result: { ok: true, value: undefined },
...
export function handleLockRoom(
  ...
  ctx.logger.info(
    `玩家 "${session.userInfo.name}" (ID: ${session.userId}) 将房间 "${room.id}" 锁定模式修改为: ${lock}`,
    { userId: session.userId },
  );
...
export function handleCycleRoom(
  ctx: HandlerCtx,
  connectionId: string,
  cycle: boolean,
  sendResponse: (response: any) => void,
  ...
  ctx.logger.info(
    `玩家 "${session.userInfo.name}" (ID: ${session.userId}) 将房间 "${room.id}" 循环状态切换为: ${cycle}`,
    { userId: session.userId },
  );
