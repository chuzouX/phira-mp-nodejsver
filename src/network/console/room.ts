...
export function broadcast(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 2) { ctx.logger.warn('[控制台] 用法: /broadcast "内容" [#ID]'); return; }
  const content = args[1];
  const target = args[2];
  const targetIds = (target && target.startsWith('#')) ? target.substring(1).split(',').map(id => id.trim()) : null;
  const rooms = ctx.roomManager.listRooms();
  let sentCount = 0;
  rooms.forEach(room => { if (!targetIds || targetIds.includes(room.id)) { ctx.protocolHandler.sendServerMessage(room.id, "【全服播报】" + content); sentCount++; } });
  ctx.logger.command(`[控制台] 播报已发送至 ${sentCount} 个房间。`);
}

export function kickPlayer(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 2) { ctx.logger.warn('[控制台] 用法: /kick {UID}'); return; }
  const uid = Number(args[1]);
  if (isNaN(uid)) { ctx.logger.warn('[控制台] 非法的用户 ID'); return; }
  const success = ctx.protocolHandler.kickPlayer(uid);
  if (success) { ctx.logger.command(`[控制台] 已踢出用户 ${uid}`); }
  else { ctx.logger.warn(`[控制台] 无法踢出用户 ${uid}，用户可能不在线。`); }
}

export function forceStart(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 2) { ctx.logger.warn('[控制台] 用法: /fstart {RID}'); return; }
  const rid = args[1];
  const success = ctx.protocolHandler.forceStartGame(rid);
  if (success) { ctx.logger.command(`[控制台] 房间 ${rid} 已强制开始对局。`); }
  else { ctx.logger.warn(`[控制台] 无法开始房间 ${rid} 的对局，请检查房间是否存在或状态是否正确。`); }
}

export function lockRoom(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 2) { ctx.logger.warn('[控制台] 用法: /lock {RID}'); return; }
  const rid = args[1];
  ...
  } else { ctx.logger.warn(`[控制台] 找不到房间 ${rid}`); }
}

export function setMaxPlayers(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 3) { ctx.logger.warn('[控制台] 用法: /maxp {RID} {人数}'); return; }
  const rid = args[1]; const count = Number(args[2]);
  if (isNaN(count)) { ctx.logger.warn('[控制台] 非法的人数限制'); return; }
  const success = ctx.protocolHandler.setRoomMaxPlayers(rid, count);
  if (success) { ctx.logger.command(`[控制台] 房间 ${rid} 最大人数已修改为 ${count}`); }
  else { ctx.logger.warn(`[控制台] 找不到房间 ${rid}`); }
}

export function closeRoom(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 2) { ctx.logger.warn('[控制台] 用法: /close {RID}'); return; }
  const rid = args[1];
  const success = ctx.protocolHandler.closeRoomByAdmin(rid);
  if (success) { ctx.logger.command(`[控制台] 房间 ${rid} 已关闭。`); }
  else { ctx.logger.warn(`[控制台] 找不到房间 ${rid}`); }
}

export function toggleMode(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 2) { ctx.logger.warn('[控制台] 用法: /tmode {RID}'); return; }
  const rid = args[1];
...
export function sendSystemMessage(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 3) { ctx.logger.warn('[控制台] 用法: /smsg {RID} {内容}'); return; }
  const rid = args[1]; const content = args[2];
...
export function bulkAction(ctx: ConsoleCtx, args: string[]): void {
  if (args.length < 2) { ctx.logger.warn('[控制台] 用法: /bulk {动作} {目标:all|#ID} [值]'); return; }
  const action = args[1]; const target = args[2] || 'all'; const value = args[3];
  const targetIds = (target !== 'all' && target.startsWith('#')) ? target.substring(1).split(',').map(id => id.trim()) : null;
  const rooms = ctx.roomManager.listRooms();
  let count = 0;
  rooms.forEach(room => {
    if (targetIds && !targetIds.includes(room.id)) return;
    switch (action) {
      case 'close_all': ctx.protocolHandler.closeRoomByAdmin(room.id); count++; break;
      case 'lock_all': if (!room.locked) ctx.protocolHandler.toggleRoomLock(room.id); count++; break;
      case 'unlock_all': if (room.locked) ctx.protocolHandler.toggleRoomLock(room.id); count++; break;
      case 'set_max_players': if (value && !isNaN(Number(value))) { ctx.protocolHandler.setRoomMaxPlayers(room.id, Number(value)); count++; } break;
    }
  });
