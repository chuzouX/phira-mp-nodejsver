...
export function listPlugins(ctx: ConsoleCtx, _args: string[]): void {
  if (!ctx.pluginManager) { ctx.logger.warn('[控制台] 插件系统未启用'); return; }
  const allPlugins = ctx.pluginManager.getAllPlugins();
  if (allPlugins.length === 0) { ctx.logger.command('[插件列表] 当前没有任何插件'); return; }
  const enabledPlugins = allPlugins.filter(p => p.enabled);
  const disabledPlugins = allPlugins.filter(p => !p.enabled);
  ctx.logger.command(`[插件列表] 共 ${allPlugins.length} 个插件 (已启用: ${enabledPlugins.length}, 已禁用: ${disabledPlugins.length})`);
  ctx.logger.command('═════════════════════════════════════════════════════════');
  if (enabledPlugins.length > 0) {
    ...
    enabledPlugins.forEach((item, index) => {
      ...
      if (plugin) {
        ...
        const deps = metadata.dependencies && metadata.dependencies.length > 0 ? ` (${metadata.dependencies.length} 个依赖)` : '';
        ctx.logger.command(`  ${index + 1}. ${metadata.name} v${metadata.version}${deps}`);
        ctx.logger.command(`     ID: ${metadata.id} | UUID: ${metadata.uuid}`);
        if (metadata.description) { ctx.logger.command(`     描述: ${metadata.description}`); }
      } else { ctx.logger.command(`  ${index + 1}. ${item.name} (未加载)`); }
      if (index < enabledPlugins.length - 1) { ctx.logger.command('     ─────────────────────────────────────────────────────'); }
    });
  }
  if (disabledPlugins.length > 0) {
    ctx.logger.command('');
    ctx.logger.command('✗ 已禁用的插件:');
    disabledPlugins.forEach((item, index) => { ctx.logger.command(`  ${index + 1}. !${item.name} (已禁用)`); });
  }
  ctx.logger.command('═════════════════════════════════════════════════════════');
...
export async function reloadPlugin(ctx: ConsoleCtx, args: string[]): Promise<void> {
  ...
  if (success) { ctx.logger.command(`[插件重载] ✓ ${pluginName} 重载成功`); }
  else { ctx.logger.warn(`[插件重载] ✗ ${pluginName} 重载失败`); }
}
...
export function showPluginInfo(ctx: ConsoleCtx, args: string[]): void {
  ...
  if (!plugin) { ctx.logger.warn(`[插件信息] 未找到插件: ${pluginName}`); ctx.logger.warn('提示: 使用 /plugins list 查看所有已加载的插件'); return; }
  const { metadata } = plugin;
  ...
  if (metadata.description) { ctx.logger.command(`描述: ${metadata.description}`); }
  if (metadata.author) { ctx.logger.command(`作者: ${metadata.author}`); }
  if (metadata.license) { ctx.logger.command(`许可证: ${metadata.license}`); }
  if (metadata.homepage) { ctx.logger.command(`主页: ${metadata.homepage}`); }
  if (metadata.repository) { ctx.logger.command(`仓库: ${metadata.repository}`); }
  if (metadata.dependencies && metadata.dependencies.length > 0) {
    ...
    metadata.dependencies.forEach(dep => {
      ...
      if (depPlugin) { ctx.logger.command(`  - ${depPlugin.metadata.name} (${depUuid})`); }
      else { const depName = typeof dep === 'string' ? '' : (dep as any).name; ctx.logger.command(`  - ${depName || depUuid}${depName ? ' (' + depUuid + ')' : ''}`); }
    });
  } else { ctx.logger.command('依赖: 无'); }
  if (metadata.tags && metadata.tags.length > 0) { ctx.logger.command(`标签: ${metadata.tags.join(', ')}`); }
  if (metadata.serverVersion) { ctx.logger.command(`要求服务器版本: ${metadata.serverVersion}`); }
...
export async function disablePlugin(ctx: ConsoleCtx, args: string[]): Promise<void> {
  ...
  if (success) { ctx.logger.command(`[插件管理] ✓ ${pluginName} 已禁用（目录已重命名为 !${pluginName}）`); }
  else { ctx.logger.warn(`[插件管理] ✗ ${pluginName} 禁用失败`); }
}
...
export async function enablePlugin(ctx: ConsoleCtx, args: string[]): Promise<void> {
  ...
  if (success) { ctx.logger.command(`[插件管理] ✓ ${pluginName} 已启用并加载`); }
  else { ctx.logger.warn(`[插件管理] ✗ ${pluginName} 启用失败`); }
}
...
export async function installPlugin(ctx: ConsoleCtx, args: string[]): Promise<void> {
  ...
  if (result.success) { ctx.logger.command(`[插件安装] ✓ ${result.message}`); }
  else { ctx.logger.warn(`[插件安装] ✗ ${result.message}`); }
}
...
export async function uninstallPlugin(ctx: ConsoleCtx, args: string[]): Promise<void> {
  ...
  if (ctx.pluginManager.getPluginByName(pluginName)) { await ctx.pluginManager.unloadPlugin(pluginName); }
  ...
  try {
    ...
    if (fs.existsSync(targetPath)) { fs.rmSync(targetPath, { recursive: true, force: true }); ctx.logger.command(`[插件管理] ✓ ${pluginName} 已卸载并删除`); }
    else { ctx.logger.warn(`[插件管理] ✗ 插件目录不存在: ${pluginName}`); }
  } catch (error: any) { ctx.logger.warn(`[插件管理] ✗ 删除失败: ${error.message}`); }
}
