...
interface AdminSecretConfig {
  adminSecret?: string;
  secretHashAlgorithm?: 'sha256' | 'sha512';
...
const pluginModule: PluginModule = {
  ...
  init(api: PluginApi) {
    ...
    // 读取配置
    const pluginConfig = api.readPluginConfig<AdminSecretConfig>() ?? {};
    ...
    // 中间件：验证 Admin Secret
    const verifyAdminSecret: express.RequestHandler = (req, res, next) => {
    ...
    // 导出中间件供其他插件使用
    (api as any).adminSecretAuthMiddleware = verifyAdminSecret;
